...
void loop() {
  ...
  sprintf(CycleTimeStamp, " %04d/%02d/%2d % 02d:%02d", now.year(), now.month(), now.day(), now.hour(), now.minute());
  ...
  temperatureCel = sensors.getTempCByIndex(0);
}
else {
  temp3231 = 0;
...
dataString = dataString + String(temperatureCel);
